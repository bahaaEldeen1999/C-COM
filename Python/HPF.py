def HPF(processes,path="../scheduler.log"):
  process_queue=sorted(processes, key=lambda x:x['priority'],reverse=False)
  time = 0
  output_file = open(path,"w")
  output_file.write("#At time x process y state arr w total z remain y wait k \n")
  while len(process_queue):
    ...
    for process in process_queue:
      if process['start'] <= time:
        ...
        servied = True
        WTA=round((process['finish']-process['start'])/process['runtime'],2)
