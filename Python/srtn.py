...
def srtn(queue):
    ...
    for i in range(len(queue)):
        queue[i]['remain'] = queue[i]['total'] = queue[i]['runtime']
        queue[i]['finish'] = queue[i]['runtime'] + queue[i]['start']
        queue[i]['wait'] = 0
    ...
    while len(queue) or len(todo):
        if run_idx != -1 and todo[run_idx]['remain'] == 0:
            ...
            todo[run_idx]['wait'] = clk - \
                (todo[run_idx]['start'] + todo[run_idx]['runtime'])
            todo[run_idx]['ta'] = clk - todo[run_idx]['start']
            todo[run_idx]['wta'] = round(
                todo[run_idx]['ta'] / (todo[run_idx]['runtime']), 2)
...
data = fileHandler('./processes.txt')
